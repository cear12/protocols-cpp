...
void TcpEchoServer::SetOnMessage(
...
void TcpEchoServer::Start() {
  ...
  if (bind(listen_fd_, reinterpret_cast<sockaddr*>(&address), sizeof(address)) <
      0) {
    int err = errno;
    ::close(listen_fd_);
    listen_fd_ = -1;
    running_ = false;
    throw std::runtime_error("TcpEchoServer: bind() failed on port " +
...
void TcpEchoServer::HandleClient(int client_fd) {
  ...
  while (running_.load()) {
    ...
    while (sent < message.size()) {
      ssize_t n =
          send(client_fd, message.data() + sent, message.size() - sent, 0);
      if (n <= 0) break;
