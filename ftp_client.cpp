...
// This file is wired into CMakeLists.txt behind `find_package(CURL)` --
// it is NOT compiled as part of this environment's own verification pass
// (no libcurl dev headers are installed here); see README.md.
#include <curl/curl.h>
...
public:
    FtpClient() : curl_(curl_easy_init()) {
        if (curl_ == nullptr) throw std::runtime_error("FtpClient: curl_easy_init() failed");
    }
    ...
    std::string downloadFile(const std::string& ftpUrl, const std::string& username = "",
        ...
        if (res != CURLE_OK) {
            throw std::runtime_error(std::string("FtpClient::downloadFile failed: ") + curl_easy_strerror(res));
        }
        return response;
    }
...
int main() {
    ...
    try {
        ...
        std::cerr << "Download failed (expected without network access): " << e.what() << "\n";
    }
}
#endif
