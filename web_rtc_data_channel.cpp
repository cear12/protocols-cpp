...
public:
    WebRtcDataChannel() {
        ...
        peerConnection_->onStateChange(
            [](rtc::PeerConnection::State state) { std::cout << "PeerConnection state: " << static_cast<int>(state) << "\n"; });
    ...
    void sendMessage(const std::string& message) {
        if (!dataChannel_ || !dataChannel_->isOpen()) {
            throw std::runtime_error("WebRtcDataChannel::sendMessage: data channel is not open");
        }
