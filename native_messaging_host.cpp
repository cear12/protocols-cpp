...
std::optional<Json::Value> readNativeMessage(std::istream& in) {
  ...
  if (!in || static_cast<std::uint32_t>(in.gcount()) != length)
    return std::nullopt;
  ...
  if (!Json::parseFromStream(builder, stream, &message, &errors))
    return std::nullopt;
  return message;
}
...
void writeNativeMessage(std::ostream& out, const Json::Value& message) {
  ...
  builder["indentation"] =
      "";  // compact, one line -- native messaging doesn't need pretty-printing
