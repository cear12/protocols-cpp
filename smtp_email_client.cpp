...
//
// Build requirement: libcurl (Ubuntu/Debian: `apt-get install libcurl4-openssl-dev`).
// Wired into CMakeLists.txt behind `find_package(CURL)`; not compiled as
// part of this environment's own verification pass -- see README.md.
#include <curl/curl.h>
...
public:
    SmtpEmailClient() : curl_(curl_easy_init()) {
        if (curl_ == nullptr) throw std::runtime_error("SmtpEmailClient: curl_easy_init() failed");
    }
    ...
    // `server` is an smtp:// or smtps:// URL, e.g. "smtps://smtp.example.com:465".
    void sendEmail(const std::string& server, const std::string& from, const std::string& to,
        ...
        curl_easy_setopt(curl_, CURLOPT_READFUNCTION, &SmtpEmailClient::readCallback);
        ...
        if (res != CURLE_OK) {
            throw std::runtime_error(std::string("SmtpEmailClient::sendEmail failed: ") + curl_easy_strerror(res));
        }
    }
...
private:
    ...
    static std::size_t readCallback(void* ptr, std::size_t size, std::size_t nmemb, void* userp) {
        auto* state = static_cast<MessageState*>(userp);
        if (state->currentLine >= state->lines.size()) return 0;  // signals "no more data"
...
int main() {
    ...
    try {
        client.sendEmail("smtps://smtp.example.com:465", "sender@example.com", "recipient@example.com",
                          "Test subject", "Test body");
        std::cout << "Email sent\n";
    } catch (const std::exception& e) {
        std::cerr << "Send failed (expected without a real SMTP server): " << e.what() << "\n";
    }
}
#endif
