// WebSocket echo server using websocketpp (https://github.com/zaphoyd/websocketpp),
// a header-only library built on Boost.Asio (or standalone Asio).
//
...
public:
    explicit WebSocketEchoServer(std::uint16_t port) : port_(port) {
        ...
        server_.set_message_handler(
            [this](websocketpp::connection_hdl hdl, WebSocketServer::message_ptr msg) { onMessage(hdl, msg); });
    }
